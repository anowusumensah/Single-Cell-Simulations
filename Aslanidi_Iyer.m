...
function output = Aslanidi_Iyer(t,y,param)
% Function written by Anthony Owusu-Mensah
% This function replaces the Hodgkin Huxley formulation of IKr
% in Aslanidi et. al (DOI: 10.1016/j.bpj.2010.02.033) with detailed
% Markov formulation in Iyer et. al(DOI: 10.1529/biophysj.104.043299)
% This fuction is passed to function objFcn_Iyer which computes 
...
A6_HERG			= param(13);
B6_HERG			= param(14);
C2H_to_C3H		= param(15); %T_Const_Herg*
C3H_to_C2H		= param(16); %T_Const_Herg*
T_Const_HERG    	= param(17);
GKr             	= param(18);
...
%Simple stimulus current
            ...
            for count=1:length(stimtimes)
                if (t > stimtimes(count) && t < stimtimes(count)+stimduration)
                    Istim = -60;
                end
            end
Cai_mM = Cai / 1000.;
...
if (V >= -40) 
    ...
    a_j = 0;
    b_j =  0.3 * exp (-2.535 * 10e-7 * V)/(1+ exp (-0.1* (V+32)));
else
    a_h = 0.135 * exp ((80+V)/-6.8);
    b_h = 0.3 * exp (-2.535 * 10e-7 * V)/(1+ exp (-0.1* (V+32)));
    a_j = ((-1.2714 * 10e5 * exp (0.2444* V) - (3.474 * 10e-5 * exp (-0.0439 * V)) * (V+37.78)))...
    /(1+exp (0.311 * (V+79.23)));
...
% Late INa Formulation
if (abs(V+47.13) < 0.05)
    a_mL = 0.32 / 0.1;
else
    a_mL = 0.32*(V+47.13)/(1-exp(-0.1*(V+47.13)));
end
b_mL   = 0.08 * exp(-V / 11);
...
% IKr channel state transitions
		   ...
		   diff_C1Herg		= C2H_to_C1H * C2Herg - C1H_to_C2H * C1Herg;
		   a1_			= C1H_to_C2H * C1Herg + C3H_to_C2H * C3Herg;
		   a2_			= (C2H_to_C1H + C2H_to_C3H) * C2Herg;
		   diff_C2Herg		= a1_-a2_;
		   a1__			= C2H_to_C3H*C2Herg + OH_to_C3H*OHerg + IH_to_C3H*IHerg;
		   a2__			= (C3H_to_IH + C3H_to_OH + C3H_to_C2H) * C3Herg ;
		   diff_C3Herg		= a1__-a2__;			
		   a1___			= C3H_to_OH * C3Herg + IH_to_OH * IHerg;
		   a2___			= (OH_to_C3H + OH_to_IH) * OHerg;
		   diff_OHerg		= a1___-a2___;		
		   a1____			= C3H_to_IH * C3Herg + OH_to_IH * OHerg;
		   a2____			= (IH_to_C3H + IH_to_OH) * IHerg;
		   diff_IHerg		= a1____-a2____;
...
% % f1 = 1.0*(1.0 - exp(-0.123 * (V + 7.0)))/0.00138 * (V + 7.0);
% % f2 = 0.00061 * (V + 10.0)/(exp(0.145 * (V + 10.0)) - 1.0);
% % tau_xr = f1 + f2;
...
% %     output(7) = diff_xr;
    ...
    output(34) = diff_Y38;
    output(35) = diff_i;
    output(36) = diff_C1Herg;
    output(37) = diff_C2Herg;
    output(38) = diff_C3Herg;
    output(39) = diff_OHerg;
    output(40) = diff_IHerg;
    output = output';
return

function out = pow(base,expo)
    out = base.^expo;
return
