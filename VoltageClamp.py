...
# -*- coding: utf-8 -*-
"""
Created on Thu Dec 15 13:10:02 2022
...
def VoltageProtocol(t, Vcm):
    #This function applies the protocol used to obtain the open probability
    # curve from Zhou et. al
    # doi: 10.1007/s10439-009-9690-5 
    # t - timepoint
    # Vcm - command Voltage
    ms2s = 1000 #convert ms to s
    thold = -80 # Holding potential
    return (t< 0*ms2s)* thold + ((0*ms2s<=t)&(t< 2*ms2s))*60 + ((2*ms2s<=t) &(t<3*ms2s))* Vcm + (t >=3*ms2s)*thold
    


def AslanidiVoltageClamp(t,y,Vcm):
    
        V = VoltageProtocol(t, Vcm)
    #def Aslanidi(t,y):    
        #global stimtimes
        #===============================================================================================
        #State variables
        ...
        b  = y[12]
        g  = y[13]
        Cai= y[14]   #Ca_buffer_Cai (umolar)
        Y0 = y[15]   #Ca_buffer_Ca_Calsequestrin (millimolar)
        Y1 = y[16]   #Ca_buffer_Ca_SL (millimolar)
        Y2 = y[17]   #Ca_buffer_Ca_SLB_SL (millimolar)
        Y3 = y[18]   #Ca_buffer_Ca_SLB_jct (millimolar)
        Y4 = y[19]   #Ca_buffer_Ca_SLHigh_SL (millimolar)
        Y5 = y[20]   #Ca_buffer_Ca_SLHigh_jct (millimolar)
        Y6 = y[21]   #Ca_buffer_Ca_SR (millimolar)
        Y7 = y[22]   #Ca_buffer_Ca_jct (millimolar )
        Y11 = y[23]
        Y23 = y[24]  #Jrel_SR_I (dimensionless)
        Y24 = y[25]  #Jrel_SR_O (dimensionless)
        Y25 = y[26]  #Jrel_SR_R (dimensionless)
        Y32 = y[27]  #cytosolic_Ca_buffer_Ca_Calmodulin (millimolar)
        Y33 = y[28]  #cytosolic_Ca_buffer_Ca_Myosin (millimolar)
        Y34 = y[29]  #cytosolic_Ca_buffer_Ca_SRB (millimolar)
        Y35 = y[30]  #cytosolic_Ca_buffer_Ca_TroponinC (millimolar)
        Y36 = y[31]  #cytosolic_Ca_buffer_Ca_TroponinC_Ca_Mg (millimolar)
        Y37 = y[32]  #cytosolic_Ca_buffer_MgMyosin (millimolar)
        Y38 = y[33]  #cytosolic_Ca_buffer_MgTroponinC_Ca_Mg (millimolar)
        i =   y[34]
        #=========================================================================================================================
        # Parameters
...
# =============================================================================
        return diff_m,diff_h,diff_j,diff_mL,diff_hL,diff_xr,diff_x,diff_y1,diff_y2,diff_xs,diff_d,diff_f,diff_b,diff_g,diff_Cai,diff_Y0,\
            diff_Y1,diff_Y2,diff_Y3,diff_Y4,diff_Y5,diff_Y6,diff_Y7,diff_Y11,diff_Y23,diff_Y24,diff_Y25,diff_Y32,diff_Y33,diff_Y34,diff_Y35,diff_Y36,\
                diff_Y37,diff_Y38,diff_i
